        ...
        if data_encoding=='csv':
            for i in range(1,len(tree.findall('layer/data'))+1):
                ...
                if i==self.layer:
                    tile_grid=[]
                    inner_tile_grid=[]
                    self.opened_file=tree.findall('layer/data')[i-1].text.split(',')
                    
                    for j in self.opened_file:
                            if j.find('\n')==-1:
                                inner_tile_grid.append(int(j))
                            elif j.find('\n')!=-1:
                                  tile_grid.append(inner_tile_grid)
                                  inner_tile_grid=[]
                                  inner_tile_grid.append(int(j.replace('\n','')))
...
- имена файлов(JPG/JPEG, PNG) в формате tuple, соответствующие ключам. Если требуется оставить тайл без картинки, вместо имени файла передать 'empty'. Если объект создан генератором,
в ключи передать 1 и 0 (1 для картинки, 0 - пустое пространство).
пример:
...
- file names(JPG/JPEG, PNG) in tuple format according to the keys. If tile needs to be left without image, pass 'empty' instead of a filename. If the object is created using the
generator, pass 1 and 0 into keys (1 for the image, 0 for empty space).
example:
cc.fill_dict((1,0),('platform2.png','empty'))
        """
